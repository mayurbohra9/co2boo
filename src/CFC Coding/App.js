...
import CreateFoodprint, { CreateFoodprintForm } from "./components/CreateFoodprint";
...
import renderSwitch, {
	...
	renderSwitchWine
} from "./components/renderSwitch";

import background from "./images/environment.jpg";



import { PieChart } from 'react-minimal-pie-chart';

import Food, { FoodList } from "./components/Food"
import './App.css';
...
function App(props) {
	...
	const [total, setTotal] = useState(renderSwitchAvocado(value1) + renderSwitchPotatoes(value2));

	const [foodprints, setFoodprints] = useState([{
		title: '',
		content: ''
	}])
	...
	useEffect(() => {
		fetch("/foodprints").then(res => {
			if (res.ok) {
				return res.json()
			}
		}).then(jsonRes => setFoodprints(jsonRes));
	})

	function handleData(e) {
		e.preventDefault()
		console.warn("all data", value1, value2)
		console.warn("CFC", renderSwitchApple(value1), renderSwitchAvocado(value2))
					...
					<Row>
						...
						<Col>
							...
							<form onSubmit={handleData}>
								...
								<br /><br />
								<h1 style={{ fontSize: 25, color: "", fontStyle: 'italic' }}>
									That is equivalent to {Math.round(calculateTotal() * 3.6)} miles driven on a typical vehicle.<br /> <img src={require("./images/car.gif").default} width="200" height="200" />
								</h1>
								...
								<br />
								<h1 style={{ fontSize: 25, color: "", fontStyle: 'italic' }}>
									To offset your carbon footprint, you will need to plant {Math.round(calculateTotal() / 21.7)} trees in a year.<br />
									<img src={require("./images/trees.gif").default} width="400" height="200" />
								</h1>
